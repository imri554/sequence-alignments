...
def getAlignment(tracebackGraph, sequences, matrix, editGraph):
    x = []
    y = []
    ...
    maxScore = np.max(editGraph)
    xCoordMax = np.where(editGraph == maxScore)[0][0]
    yCoordMax = np.where(editGraph == maxScore)[1][0]

    #the crucial difference between local and global alignment
    j = xCoordMax
    i = yCoordMax
    ...
    print(''.join(y[::-1]))
    print(''.join(x[::-1]))
    print(editGraph[xCoordMax][yCoordMax])
...
#why is it minus three???
    ...
    for j in range(1, len(sequences[0]) + 1):
        for i in range(1, len(sequences[1]) + 1):
            editGraph[j][i] = max(0, editGraph[j-1][i-1] + int(scoreTheMatch(sequences[0][j-1], sequences[1][i-1])),
